<template>
  <div>
    <div class="header">
      <Input placeholder="输入房号"
             style="width: 300px"
             v-model="fh" />
      <Button type="success"
              @click="getRoomByfh()">搜索</Button>
    </div>
    <div>
      <Table border
             :columns="columns1"
        ...
        <template slot-scope="{ row, index }"
          ...
          <Button type="warning"
                  size="small"
                  @click="modifyData(index)">修改</Button>
          <Button type="primary"
                  :disabled="row.yrz == '是'"
                  size="small"
                  @click="confirm(row.roomOrderUuid)">确认入住</Button>
          <Button type="error"
                  size="small"
                  @click="okqxrz(row.roomOrderUuid)">取消入住</Button>
        </template>
      </Table>
    </div>
    <div class="page">
...
<script>
import { roomOrder, roomOrderC, roomOrderD, fhGetRoom } from '@/network/user.js'
export default {
  data() {
    return {
      fh: '',
