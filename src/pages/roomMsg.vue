<template>
  <div>
    ...
    <div>
      <Tag @on-change="kf"
           checkable
           color="primary">空房</Tag>
      <Tag @on-change="zz"
           checkable
           color="success">在住</Tag>
      <Tag @on-change="ty"
           checkable
           color="error">停用</Tag>
    </div>
          ...
          <p slot="title"
            ...
            <Dropdown trigger="click"
              ...
              <DropdownMenu slot="list">
                <DropdownItem>
                  <p class="drop-p"
                     @click="clickchakan(item.roomUuid)">查看</p>
                </DropdownItem>
                <DropdownItem>
                  <p class="drop-p"
                     @click="clickxiugai(item.roomUuid)">修改房价</p>
                </DropdownItem>
                <DropdownItem v-if="item.fjzt == '在住' ? true : false">
                  <p class="drop-p"
                     @click="clickxuzu(item.roomUuid)">续住</p>
                </DropdownItem>
                <DropdownItem v-if="item.fjzt == '在住' ? true : false">
                  <p class="drop-p"
                     @click="clicktuifang(index)">退房</p>
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
          </p>
          <p @click="clickchakan"
             class="card-item-p">房型：{{ item.fjlx }}</p>
